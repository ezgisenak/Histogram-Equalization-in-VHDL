...
entity Histogram is
    ...
    Port (
        ...
        histogram             : out STD_LOGIC_VECTOR(DATA_WIDTH - 1 downto 0); 
        valid                 : in STD_LOGIC;
...
architecture Behavioral of Histogram is
    ...
    signal temp_addr_buffer           : STD_LOGIC_VECTOR(4 * ADDR_WIDTH - 1 downto 0);
    ...
    type t_state is (START, WRITE, READ, IDLE, HIST_EQ);
        ...
        if reset_sig = '1' then
            ...
            case STATE is
                ...
                when HIST_EQ => 
                    if valid = '1' then
                        bram_addr_rd  <= pixel_in;
                        sum <= std_logic_vector(to_unsigned(to_integer(unsigned(sum + bram_dout)) * to_integer(unsigned(no_bins)), DATA_WIDTH));
                        bram_addr_wr  <= temp_addr_buffer(3 * ADDR_WIDTH - 1 downto 2 * ADDR_WIDTH);
                        bram_din <= sum;
                        temp_addr_buffer(ADDR_WIDTH - 1 downto 0) <= pixel_in;
                        temp_addr_buffer(4 * ADDR_WIDTH - 1 downto ADDR_WIDTH) <= temp_addr_buffer(3 * ADDR_WIDTH - 1 downto 0);
